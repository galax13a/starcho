...
>
    <div class="flex w-full items-center justify-end gap-2 overflow-x-auto md:flex-nowrap">
        <x-starcho-btn-excel
            ...
            :requireSelectionMessage="__('contacts.notify.no_selection')"
        />
    ...
    <div
        ...
        class="inline-flex items-center gap-2 self-end rounded-xl border border-sky-200 dark:border-sky-700/40 bg-sky-50 dark:bg-sky-900/20 px-3 py-1.5"
    >
        <span class="inline-flex items-center h-8 px-3 rounded-lg bg-white/80 dark:bg-zinc-900/70 text-xs font-semibold text-sky-700 dark:text-sky-300">
            <span x-text="selected.length"></span>
            <span class="ml-1">{{ __('contacts.bulk_selected') }}</span>
        </span>
        ...
        <button
            ...
            @click="window.Starcho.confirm({
                ...
                message: @js(__('contacts.bulk_delete_confirm')),
                okText: @js(__('js.delete.ok')),
                cancelText: @js(__('js.confirm.cancel')),
                onConfirm: () => $wire.deleteSelected(),
            })"
            class="inline-flex items-center gap-1.5 h-8 px-3 rounded-lg bg-rose-600 hover:bg-rose-700 text-sky-700 dark:text-sky-300 text-xs font-medium transition"
        >
            ...
            {{ __('contacts.bulk_delete_selected') }}
        </button>
        ...
        >
            ...
            {{ __('contacts.bulk_clear_selection') }}
        </button>
    </div>
</div>
