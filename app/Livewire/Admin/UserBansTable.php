...
final class UserBansTable extends PowerGridComponent
{
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('ban_reason')
            ->add('ban_reason_short', fn (User $u) => $u->ban_reason ? Str::limit($u->ban_reason, 50) : '—')
            ->add('banned_until')
    ...
    public function actions(User $row): array
    {
        ...
        if (! $row->is_banned) {
            $actions[] = Button::make('ban', __('admin_ui.users_ban.actions.ban'))
                ->class('sa-btn sa-btn-danger sa-btn-sm')
                ->dispatch('openBanModal', ['userId' => $row->id, 'userName' => $row->name]);
        } else {
            $actions[] = Button::make('unban', __('admin_ui.users_ban.actions.unban'))
